...
import it.unimi.di.big.mg4j.query.nodes.CheckForSelectQueryVisitor;
...
import it.unimi.di.big.mg4j.query.nodes.Queries;
...
import it.unimi.di.big.mg4j.query.parser.SimpleCharStream;
import it.unimi.di.big.mg4j.query.parser.SimpleParserConstants;
import it.unimi.di.big.mg4j.query.parser.SimpleParserTokenManager;
...
import it.unimi.di.big.mg4j.query.parser.Token;
import it.unimi.di.big.mg4j.query.parser.ParseException;

/*		 
 * MG4J: Managing Gigabytes for Java (big)
 *
 * Copyright (C) 2006-2012 Paolo Boldi and Sebastiano Vigna
 *
...
/**
 * A simple parser that transform a query string into a
 * {@linkplain it.unimi.di.big.mg4j.query.nodes query}.
 *
 * <P>
 * The parser supports multiple indices. You must provide a set of
 * <em>index aliases</em> that the user will use to select indices, and the name
 * of the default index alias to be used. After that, you parse a query by using
 * the {@link #parse(String)} method.
 *
 * <P>
 * The parser generated by JavaCC for this class will break terms using
 * operators and nonspace-to-space transitions. Operators can be included in
 * terms, if needed, using the backslash, which acts as an escape character, and
 * makes the next character (usually an operator) a standard character. The
 * syntax of the parser can be seen in action in the documentation of the
 * package {@link it.unimi.di.big.mg4j.search}.
 * 
 * <P>
 * The parser returns a {@link it.unimi.di.big.mg4j.query.nodes.Query}&mdash;an
 * abstract representation of the query string that can be turned later into a
 * {@link it.unimi.di.big.mg4j.search.DocumentIterator}.
 *
 * <P>
 * If a {@link TermProcessor} is specified, it will be applied to the terms
 * found in the query string. This can lead to transformations (e.g.,
 * downcasing) or generate an exception if the query string contains terms
 * filtered by the term processor.
 *
...
public class SimpleParser implements QueryParser, SimpleParserConstants {
	...
	private final static Logger LOGGER = Logger.getLogger(SimpleParser.class);
	 ...
	 *            the set of index aliases.
	 * @param defaultIndex
	 *            the default index alias to be used when parsing the query.
	 * @param termProcessors
	 *            a map from index aliases to the corresponding term processor,
	 *            or <code>null</code> for no term processing.
	 */
	public SimpleParser(final Set<String> indices, final String defaultIndex,
			final Map<String, ? extends TermProcessor> termProcessors) {
		this(new java.io.StringReader("")); // Just for initialization
											// purposes...
	 ...
	 *
	 * <P>
	 * Parsers created by this constructor allow only nameless access, both in
	 * the query and in the interval-iterator methods.
	 */
	public SimpleParser() {
	...
	public SimpleParser(final TermProcessor termProcessor) {
		this(ObjectSets.singleton(""), "", Object2ObjectMaps.singleton("",
				termProcessor));
	}
	...
	private final char[] SPECIAL_CHARACTER = new char[] { ' ', '&', '$', '<',
			'|', '!', '(', ')', '{', '}', '[', ']', ',', '\"', ':', '~', '#',
			'-', '+', '*', '\\', '^', '\u2227', '\u2228', '\u22a4', '\u22a5', };
	private final String[] SPECIAL_CHARACTER_REPLACEMENT = new String[] {
			"\\ ", "\\&", "\\$", "\\<", "\\|", "\\!", "\\(", "\\)", "\\{",
			"\\}", "\\[", "\\]", "\\,", "\\\"", "\\:", "\\~", "\\#", "\\-",
			"\\+", "\\*", "\\\\", "\\^", "\\\u2227", "\\\u2228", "\\\u22a4",
			"\\\u22a5", };
	...
	public String escape(final String token) {
		return new MutableString(token).replace(SPECIAL_CHARACTER,
				SPECIAL_CHARACTER_REPLACEMENT).toString();
	}
	...
	public Query parse(final String text) throws QueryParserException {
		...
		try {
			...
			try {
				visitor.prepare();
				if (query.accept(visitor) == null)
					throw new ParseException(visitor.errorMessage);
	 ...
	 * 
	 * @param indexAlias
	 *            the index alias for the default index to be used for the query
	...
	final public Query remappedQuery(String indexAlias) throws ParseException {
		...
		case OPEN_REMAP:
			...
			if (DEBUG)
				System.err.println("Building remapped query");
		...
		if (externalIndices.isEmpty()) {
			if (true)
				return query;
		}
		{
			if (true)
				return new Remap(query,
						internalIndices.toArray(new String[internalIndices
								.size()]),
						externalIndices.toArray(new String[externalIndices
								.size()]));
		}
		throw new Error("Missing return statement in function");
	}
	...
	final public Query differenceQuery(String indexAlias) throws ParseException {
		...
		case MINUS:
			...
			if (DEBUG)
				System.err.println("Building difference query");
			switch ((jj_ntk == -1) ? jj_ntk() : jj_ntk) {
			case OPEN_ENLARGE:
		...
		}
		{
			if (true)
				return new Difference(minuend, subtrahend,
						leftMargin == null ? 0 : Integer
								.parseInt(leftMargin.image),
						rightMargin == null ? 0 : Integer
								.parseInt(rightMargin.image));
		}
		throw new Error("Missing return statement in function");
	}
	...
	final public Query orQuery(String indexAlias) throws ParseException {
		...
		label_2: while (true) {
			...
			jj_consume_token(OR);
			if (DEBUG) {
				System.err.println("Building OR query");
			}
			try {
				res = orderedAndQuery(indexAlias);
				qrm.add(res);
			} catch (Exception e) {
				LOGGER.warn("Term was not found", e);
			}
		}
		if (qrm.size() == 1) {
			if (true)
	...
	final public Query andQuery(String indexAlias) throws ParseException {
		...
		if (DEBUG)
			System.err.println("Building AND query: " + res);
		qrm.add(res);
		label_3: while (true) {
			switch ((jj_ntk == -1) ? jj_ntk() : jj_ntk) {
			case AND:
	...
	final public Query alignQuery(String indexAlias) throws ParseException {
		...
		case ALIGN:
			...
			if (DEBUG)
				System.err.println("Building aligned query: " + alignee);
	...
	final public Query multiTermQuery(String indexAlias) throws ParseException {
		...
		if (DEBUG)
			System.err.println("Building multiterm query: " + res);
		qrm.add(res);
		label_5: while (true) {
			switch ((jj_ntk == -1) ? jj_ntk() : jj_ntk) {
			case PLUS:
			...
			if (qrm.size() == 1
					&& !(res instanceof Term)
					&& !((res instanceof Weight) && (((Weight) res).query instanceof Term))) {
				if (true)
					throw new ParseException("Expecting a term instead of "
							+ res);
			}
			res = atomicQuery(indexAlias);
			if (DEBUG)
	...
	final public Query quotedQuery(String indexAlias) throws ParseException {
		...
		if (qrm.size() == 1 && !holeSeen) {
			if (res == null) {
				if (true)
					throw new ParseException(
							"You must specify at least one term within quotes");
			}
			{
				if (true)
					return res;
		...
		{
			if (true)
				return new Consecutive(qrm.toArray(Queries.EMPTY_ARRAY),
						holeSeen ? gaps.toIntArray() : null);
		}
		throw new Error("Missing return statement in function");
	}

	/**
	 * Starting rule for an atomic query. May be either an atomic simple query
	 * or a quoted query. It can optionally contain an index selector and a
	 * low-pass limit.
	 * 
	 * @param indexAlias
	 *            the index alias for the default index to be used for the query
	...
	final public Query atomicQuery(String indexAlias) throws ParseException {
		...
		if (negative && annotation) {
			if (true)
				throw new ParseException("You cannot negate annotations");
		}
		if (negative)
			res = new Not(res);
		if (annotation)
			res = new Annotation(res);
		if (newIndexAlias != indexAlias)
			res = new Select(newIndexAlias, res);
		switch ((jj_ntk == -1) ? jj_ntk() : jj_ntk) {
		case TILDA:
			...
			try {
			...
			} catch (NumberFormatException e) {
				{
					if (true)
						throw new ParseException(
								"Expecting a positive number, found " + t.image
										+ " instead");
				}
			}
			if (DEBUG)
				System.err.println("Building low-pass filter with threshold "
						+ limit);
	 ...
	 * @return the result of the query.
	 */
	final public Query atomicSimpleQuery(String indexAlias)
		...
		case WORD:
			...
			if (!indices.contains(indexAlias)) {
				if (true)
					throw new ParseException("Index \"" + indexAlias
							+ "\" does not exist");
			}

			if (s != null && p != null) {
				if (true)
					throw new ParseException(s + " and " + p
			...
			if (s == null) {
				// Before passing the word to anyone, we pass it through the
				// index term processor.
				...
				if (p != null) {
					if (termProcessors != null
							&& !termProcessors.get(indexAlias).processPrefix(
									term)) {
						if (true)
							throw new ParseException("Prefix " + term
									+ " has not been indexed");
					}
				} else {
					if (termProcessors != null
							&& !termProcessors.get(indexAlias)
									.processTerm(term)) {
						if (true)
							throw new ParseException("Term " + term
									+ " has not been indexed");
					}
				}
			}
			...
			if (p == null) {
				if (s != null) {
					try {
						...
						{
							if (true)
								throw new ParseException("Malformed number: "
										+ word);
						}
					}
				} else
		...
		case OPEN_RANGE:
			...
			res = new Range(s != null ? s.toString() : null,
		...
		case OPEN_WEIGHT:
			...
			try {
			...
			} catch (NumberFormatException e) {
				{
					if (true)
						throw new ParseException(
								"Expecting a nonnegative number, found "
										+ t.image + " instead");
				}
			}
			if (DEBUG)
				System.err.println("Building weight: " + weight);
	...
	private static void jj_la1_0() {
		jj_la1_0 = new int[] { 0x1000000, 0x10000, 0x80000, 0x8000000, 0x80,
				0x70601520, 0x20, 0x40, 0x200, 0x4000000, 0x70001400,
				0x70001400, 0x70001400, 0x100, 0x200000, 0x70401400, 0x2000000,
				0x10000000, 0x80000000, 0x0, 0x0, 0x70001400, 0x4000, };
	}

	private static void jj_la1_1() {
		jj_la1_1 = new int[] { 0x0, 0x0, 0x0, 0x0, 0x0, 0x4, 0x0, 0x0, 0x0,
				0x0, 0x5, 0x5, 0x5, 0x0, 0x0, 0x4, 0x0, 0x0, 0x0, 0x4, 0x4,
				0x4, 0x0, };
	}
	...
	static private final class LookaheadSuccess extends java.lang.Error {
		 ...
		 * 
		 */
		private static final long serialVersionUID = 6421194751967873193L;
	}
